...
const MOCK_CAMPAIGNS = [
  { id: 2, name: "Dubai Luxury Real Estate Q2", objective: "sales", platform: "instagram", status: "active", budget_daily: 75, targeting_json: { ad_format: "reel" } },
  { id: 4, name: "NRI Investment Advisory", objective: "lead_gen", platform: "instagram", status: "active", budget_daily: 50, targeting_json: { ad_format: "story" } }
];
...
const NeoVibePanel = () => {
  ...
  const fetchCampaigns = async () => {
    setLoading(true);
    try { const r = await fetch('/api/neovibe/instagram-campaigns'); if (r.ok) setCampaigns(await r.json()); } catch (e) {} finally { setLoading(false); }
  };
  ...
  const handleCreateCampaign = async () => {
    try {
      ...
      setCampaigns(p => [...p, { id: Date.now(), ...values, status: 'draft', platform: 'instagram', targeting_json: {} }]);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div><h2 style={{ margin: 0 }}>NeoVibe Campaigns</h2><p style={{ margin: '4px 0 0', color: '#999' }}>Manage your Instagram advertising campaigns</p></div>
