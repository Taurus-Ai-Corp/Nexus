#!/usr/bin/env python3
"""Create v03 synthetic damage preview for IMG-AD-20260609-0001.

Uses a perspective-warped lid damage texture plus a shaded dent on the top surface.
Still explicitly watermarked as synthetic preview-only content.
"""
...
EDIT_VERSION = 'damage-cracked-lid-dented-top-v04'
...
def soft_line(draw: ImageDraw.ImageDraw, p1, p2, color, width=6, blur=1.8):
...
def add_dent(img: Image.Image, cx, cy, rx, ry, intensity=0.22) -> None:
    ...
    arr[mask] = arr[mask] * (1.0 - intensity * shade) + np.array([28, 30, 34, 255]) * (intensity * shade)
    img.paste(Image.fromarray(arr.astype('uint8')), None)

    draw = ImageDraw.Draw(img, 'RGBA')
    draw.ellipse((cx - rx, cy - ry, cx + rx, cy + ry), outline=(0, 0, 0, 80), width=9)
    draw.ellipse((cx - rx + 10, cy - ry + 8, cx + rx - 8, cy + ry - 7), outline=(255, 255, 255, 85), width=3)


def add_scratches(img: Image.Image, seed: int, count: int = 28) -> None:
...
def create_lid_damage_texture(size=(1000, 1000), seed=20260609):
    rng = random.Random(seed)
    ...
    # Main crack network
    starts = [(int(w * 0.45), int(h * 0.25)), (int(w * 0.55), int(h * 0.35)), (int(w * 0.35), int(h * 0.55))]
    for sx, sy in starts:
        x, y = sx, sy
        for _ in range(9):
            nx = min(max(x + rng.randint(-90, 120), 40), w - 40)
            ny = min(max(y + rng.randint(-70, 130), 40), h - 40)
            width = rng.randint(3, 6)
            soft_line(draw, (x, y), (nx, ny), (0, 0, 0, 105), width + 5, 2.0)
            soft_line(draw, (x, y), (nx, ny), (15, 18, 22, 230), width, 1.0)
            soft_line(draw, (x, y), (nx, ny), (245, 250, 255, 150), max(1, width // 2), 0.8)
            if rng.random() < 0.45:
                bx = min(max((x + nx) // 2 + rng.randint(-120, 120), 30), w - 30)
                by = min(max((y + ny) // 2 + rng.randint(-100, 120), 30), h - 30)
                soft_line(draw, (nx, ny), (bx, by), (0, 0, 0, 85), max(2, width - 1), 1.8)
                soft_line(draw, (nx, ny), (bx, by), (15, 18, 22, 210), max(2, width - 1), 1.0)
            x, y = nx, ny

    # Broken-glass shard highlights, low opacity
    for _ in range(12):
        cx = rng.randint(180, w - 180)
        cy = rng.randint(160, h - 160)
        pts = []
        for a in np.linspace(0, 2 * np.pi, 3, endpoint=False):
            rr = rng.uniform(35, 95)
            pts.append((int(cx + rr * np.cos(a)), int(cy + rr * np.sin(a))))
        alpha = rng.randint(25, 60)
        draw.polygon(pts, fill=(235, 248, 255, alpha), outline=(255, 255, 255, min(120, alpha + 40)))
        draw.line(pts + [pts[0]], fill=(8, 12, 16, min(130, alpha + 70)), width=2)

    # Small chips and impact points
    for _ in range(18):
        x = rng.randint(120, w - 120)
        y = rng.randint(120, h - 120)
        r = rng.randint(3, 9)
        draw.ellipse((x - r, y - r, x + r, y + r), fill=(0, 0, 0, rng.randint(70, 130)))
        draw.ellipse((x - r + 2, y - r + 2, x + r - 1, y + r - 1), fill=(255, 255, 255, rng.randint(35, 80)))

    return tex.filter(ImageFilter.GaussianBlur(0.35))


def add_perspective_overlay(img: Image.Image, quad, texture: Image.Image, opacity=0.92):
    tex = texture.copy()
    if opacity < 1:
        alpha = tex.getchannel('A').point(lambda p: int(p * opacity))
        tex.putalpha(alpha)
    warped = tex.transform(img.size, Image.Transform.QUAD, data=quad, resample=Image.Resampling.BICUBIC)
...
def main() -> None:
    ...
    # Dent on the white top/control area.
    add_dent(img, int(w * 0.55), int(h * 0.13), int(w * 0.11), int(h * 0.045), intensity=0.24)
    add_dent(img, int(w * 0.45), int(h * 0.16), int(w * 0.055), int(h * 0.032), intensity=0.18)

    # Lid damage is perspective-warped onto the transparent lid plane.
    lid_quad = [
        int(w * 0.18), int(h * 0.23),
        int(w * 0.84), int(h * 0.21),
        int(w * 0.88), int(h * 0.82),
        int(w * 0.13), int(h * 0.86),
    ]
    tex = create_lid_damage_texture((1100, 1100), seed=20260609)
    add_perspective_overlay(img, lid_quad, tex, opacity=0.90)

    add_scratches(img, seed=20260609, count=26)
