...
def load_data():
    try:
        with open('/workspace/data/quick_diagnostics_results.json') as f:
            quick_data = json.load(f)
        with open('/workspace/data/network_analysis_results.json') as f:
...
def create_performance_chart(quick_data, network_data):
    ...
    if network_data and 'concurrent_load' in network_data:
        ...
        sorted_data = sorted(zip(concurrency_levels, success_rates, avg_response_times, strict=False))
        concurrency_levels, success_rates, avg_response_times = zip(*sorted_data, strict=False)
...
def create_diagnostic_summary():
    ...
    for i, (bar, result) in enumerate(zip(bars, results, strict=False)):
