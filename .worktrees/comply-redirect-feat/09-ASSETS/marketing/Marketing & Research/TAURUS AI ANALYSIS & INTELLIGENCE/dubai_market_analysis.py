...
def create_pricing_analysis():
    ...
    # Add value labels
    for i, (min_val, max_val) in enumerate(zip(min_prices, max_prices)):
    ...
    avg_cpc = [(min_val + max_val) / 2 for min_val, max_val in zip(cpc_min, cpc_max)]
    errors = [[(avg - min_val, max_val - avg)] for avg, min_val, max_val in zip(avg_cpc, cpc_min, cpc_max)]
    ...
    # Add range labels
    for i, (min_val, max_val, avg_val) in enumerate(zip(cpc_min, cpc_max, avg_cpc)):
...
def create_roi_automation_analysis():
    ...
    for i, (bar, value, metric) in enumerate(zip(bars1, roi_metrics, metric_types)):
    ...
    for i, (cost, roi, level) in enumerate(zip(monthly_costs, roi_multipliers, investment_levels)):
