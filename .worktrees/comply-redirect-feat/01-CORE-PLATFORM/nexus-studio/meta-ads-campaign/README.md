# Nexus — Meta Ads Campaign Blueprint

**Status**: BLUEPRINT ONLY — no ads will run. All CLI commands ship `--status PAUSED`.
**Owner**: TAURUS AI Corp / Nexus operating brand
...
3. **Landing/destination URL is unconfirmed.** Assumes `https://nexus.taurusai.io`. Confirm the canonical URL, that the Pixel + Conversion API are firing, and that the page loads <2s on mid-tier mobile in Kochi/Dubai.
...
- **Provision a separate Meta ad account.** Don't co-mingle Nexus spend with Mater Maria — different audiences, different attribution windows, different LTV. Set `AD_ACCOUNT_ID` to the new `act_NNNNN` before running `cli-commands.sh`.
