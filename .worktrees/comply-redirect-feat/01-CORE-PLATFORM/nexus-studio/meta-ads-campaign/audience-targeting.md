# Nexus — Audience Targeting

Three saved audiences. All exclude an uploaded **Customer List** custom audience (`nexus_existing_users.csv` — emails of approved users + paying customers). Existing-user list must be hashed before upload (Meta does this client-side via the standard hashed-CSV flow).
...
```jsonc
{
  ...
  "exclusions": {
    "custom_audiences": ["nexus_existing_users"],
...
```jsonc
{
  ...
  "estimated_reach_note": "Likely 200K-350K monthly active. Higher CPM than Kerala (3x typical). The expat-from-India layer is intentional — that's a high-fit slice for Nexus given Kerala-Gulf design freelance corridor."
}
```
...
```jsonc
{
  "name": "Nexus Warm — Site Visitors + Video Engagers (30d)",
  "type": "custom_audience",
  "sources": [
    {
      "type": "website",
      "rule": "URL contains: nexus.taurusai.io",
      "retention_days": 30
    },
    {
      "type": "video",
      "rule": "Watched 25% or more",
      "video_ids": ["<all Nexus video creatives from this campaign>"],
      "retention_days": 30
    },
    {
      "type": "instagram_engagement",
      "rule": "Engaged with Nexus IG profile or post",
      "retention_days": 30
    }
  ],
  "geo": {
  ...
  "exclusions": {
    "custom_audiences": [
      "nexus_existing_users",
      "nexus_lead_form_completed_30d"
    ]
  },
...
`nexus_existing_users.csv` — minimum columns (Meta will hash on upload):
