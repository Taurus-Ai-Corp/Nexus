...
description: Build the dedicated integration settings page that lets end users manage their connected integrations — view connection health, reconnect/relink, configure per-integration settings (sync scope, field mappings, data filters, writeback behavior), and track setup completeness. Use as Step 2 of post-connection implementation. Use when a developer says "build settings page", "integration settings", "configure integration", "sync scope", "field mappings", "writeback configuration", or "setup completeness" for a Merge integration.
license: MIT
