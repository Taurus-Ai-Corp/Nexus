...
> I'm the sync-implement-polling skill. I'll wire up a single scheduled job that detects both initial and subsequent Merge syncs by calling `GET /sync-status`. This is great for local dev and as a production fallback alongside webhooks.
...
- **Polling alone is not production-grade**: layer in `sync-implement-webhooks` for real-time detection. The two are designed to coexist.
...
**FIX:** Increase the base poll interval. Apply exponential backoff with jitter on 429. Consider running webhooks (`sync-implement-webhooks`) as the primary trigger and slowing polling to a low-frequency safety net.
