...
- **Public webhook URL**: production URL, staging URL, or local tunnel (ngrok / cloudflared)? Confirm one is available before continuing — without a reachable URL, polling is the only option (see `sync-implement-polling`).
...
- **Add polling as a fallback** (`sync-implement-polling`): webhook delivery occasionally lags or drops. Polling on a slow cadence catches what webhooks miss.
