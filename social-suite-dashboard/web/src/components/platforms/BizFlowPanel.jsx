...
const BizFlowPanel = () => {
  ...
  const fetchCampaigns = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/api/bizflow/meta-campaigns?${params.toString()}`);
      setCampaigns(response.data);
    } catch (error) {
      console.error('Error fetching BizFlow campaigns:', error);
  ...
  const handleCreateCampaign = async () => {
    try {
      ...
      const response = await axios.post('/api/bizflow/meta-campaigns', {
        ...values,
        platform: 'meta',
        status: 'draft',
        start_date: new Date(),
      ...
      message.success('BizFlow campaign created successfully!');
      form.resetFields();
      fetchCampaigns();
    } catch (error) {
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 24 }}>
        <h2>BizFlow Meta Campaigns</h2>
        <p>Manage your Facebook and Meta advertising campaigns</p>
      </div>
      ...
      {loading ? (
        ...
        <Form form={form} layout="vertical">
          ...
          <Item label="Targeting (JSON)">
            <Input.TextArea 
              rows={4} 
              placeholder='{"location": "Detroit", "age_range": "25-45"}' 
              name="targeting_json"
            />
          </Item>
      ...
      {campaigns.length === 0 && !loading && (
        <div style={{ textAlign: 'center', padding: 40 }}>
          <p>No BizFlow campaigns found. Create your first campaign above.</p>
        </div>
      )}
    </div>
  );
