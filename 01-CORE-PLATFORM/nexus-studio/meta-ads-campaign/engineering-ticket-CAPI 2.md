# ENG-NV-001 — Implement Meta Conversions API (CAPI) for Nexus with Quality-Gated Optimization
...
**Blocks:**
- Nexus paid acquisition launch (Meta + Instagram)
- Nexus lookalike audience generation (lookalike-1% off `account_activated`, not `lead_submitted`)
...
TypeScript types for `EventData`, `UserData`, `RawUserData`, `HashedUserData`, `CapiResult`, `NexusCustomEvent` (union of `'account_approved' | 'account_activated'`).
...
Likely paths (confirm during scoping — the studio app structure under `01-CORE-PLATFORM/nexus-studio/Nexus_DESIGN_LOUNGE/BIizflow_supernova_OCT_8/src/app/` lacks a signup route yet):
...
4. Fire `sendCapiEvent('Lead', { content_name: 'nexus_signup', value: 0, currency: 'USD' }, hashedUserData, { eventId })`
...
3. **Production canary (50% of signups):** PostHog feature flag `meta-capi-enabled`. Nexus signup volume is not yet high enough to justify a 10% canary — go straight to 50%. `META_TEST_EVENT_CODE` UNSET in production
