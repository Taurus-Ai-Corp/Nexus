...
# Previous hardcoded tokens have been rotated and removed.
# Set these in your .env file:
#   WEBFLOW_ACCESS_TOKEN=your-token
...
if not all([WEBFLOW_ACCESS_TOKEN, WEBFLOW_SITE_ID]):
    import warnings

    warnings.warn(
        "Webflow credentials not configured. Set WEBFLOW_ACCESS_TOKEN and WEBFLOW_SITE_ID in .env"
    )
...
class WebflowAPIClient:
    ...
    def __init__(self, api_token=None):
        self.api_token = (
            api_token or os.getenv("WEBFLOW_ACCESS_TOKEN") or WEBFLOW_ACCESS_TOKEN
        )
    ...
    def _mock_get(self, endpoint, params=None):
        ...
        if "/sites" in endpoint:
            return {
                "sites": [
                    {
                        "id": "taurus-ai-web-land-dash-2025",
                        "name": "TAURUS AI Web Land Dash",
                    }
                ]
            }
...
class WebLandDashWebflowIntegration:
            ...
            for dashboard in dashboards:
                try:
                    ...
                    results["error_details"].append(
                        {"dashboard": dashboard, "error": str(e)}
                    )
        ...
        try:
            ...
            for widget in widgets:
                try:
                    ...
                    results["errors"] += 1
                    results["error_details"].append({"widget": widget, "error": str(e)})
        ...
        try:
            ...
            for user in users:
                try:
                    ...
                    results["errors"] += 1
                    results["error_details"].append({"user": user, "error": str(e)})
    ...
    def sync_platform_analytics(
        ...
        """
        try:
            results = {"logged": 0, "errors": 0, "error_details": []}
            ...
            for analytics in analytics_data:
                try:
                    ...
                    results["errors"] += 1
                    results["error_details"].append(
                        {"analytics": analytics, "error": str(e)}
                    )
    ...
    def update_dashboard_layout(
        ...
        try:
            # Update dashboard layout
            updated_dashboard = self.dash_service.update_dashboard_layout(
                dashboard_id, layout_data
            )
        ...
        try:
            ...
            report = {
                ...
                "active_dashboards": len(
                    [
                        d
                        for d in dashboards.get("items", [])
                        if d.get("fieldData", {}).get("is-active", False)
                    ]
                ),
                "active_users": len(
                    [
                        ...
                        if u.get("fieldData", {}).get("is-active", False)
                    ]
                ),
