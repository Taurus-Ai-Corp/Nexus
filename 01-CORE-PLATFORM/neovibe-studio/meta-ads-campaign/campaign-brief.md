# NeoVibe — Campaign Brief (Media Buyer Plan)
...
**Why instant form, not Conversions-to-trial**: NeoVibe's signup is admin-gated (Clerk approval queue). Until free-trial signup is end-to-end verified — Clerk approval flow + Stripe/billing wired + landing page Pixel events firing — running on a Conversions objective would burn budget feeding the algo half-broken events. Instant form is the conservative ship. Upgrade path: once trial signup is live and 50+ trials have fired through the Pixel, duplicate the campaign as Conversions-objective and run them side-by-side for two weeks before retiring the lead form.
...
- Operating brand on creatives = "NeoVibe" (NOT "TAURUS AI Corp" or "FZCO"). Customer-facing.
- Lead form privacy URL must point to NeoVibe's privacy policy, not the corporate FZCO entity.
