...
class LocalAIRouter:
    ...
    def __init__(self):
        ...
        self.request_count = {"local": 0, "cloud": 0, "fine_tuned": 0}
        self.last_model_used = None
        self.is_ready = False
        
        # Fine-tuned model registry
        self.fine_tuned_models = {}  # {model_id: {base_model, path, metadata}}
        self.fine_tuned_preference = os.getenv("PREFER_FINE_TUNED", "true").lower() == "true"
    ...
    async def initialize(self):
        ...
        try:
            ...
            await self.load_fine_tuned_models()
    ...
    async def route_request(self, 
        ...
        if model_preference != "auto":
            ...
            if model_preference in self.fine_tuned_models:
                return f"fine_tuned:{model_preference}"
            elif model_preference in self.local_models:
        ...
        # Check for fine-tuned model match
        if self.fine_tuned_preference:
            fine_tuned_model = self._find_matching_fine_tuned_model(task_type, complexity)
            if fine_tuned_model:
                logger.info(f"🎯 Using fine-tuned model: {fine_tuned_model}")
                return f"fine_tuned:{fine_tuned_model}"
    ...
    async def chat_completion(self, 
        ...
        try:
            if model_type == "fine_tuned":
                return await self._fine_tuned_chat_completion(
    ...
    async def load_fine_tuned_models(self):
        ...
        try:
            ...
            registry_path = Path(__file__).parent.parent / "services" / "gen-ai-finetuning" / "deployment" / "model_registry.py"
            if registry_path.exists():
                # Load fine-tuned models from registry
                # This is a placeholder - actual implementation would use ModelRegistry
                logger.info("🔍 Scanning for fine-tuned models...")
                # Fine-tuned models would be loaded here
                logger.info(f"✅ Loaded {len(self.fine_tuned_models)} fine-tuned models")
            else:
                logger.info("📦 Fine-tuning service not found, skipping fine-tuned model loading")
        except Exception as e:
            logger.warning(f"⚠️ Could not load fine-tuned models: {e}")
    ...
    def register_fine_tuned_model(
        ...
        logger.info(f"✅ Registered fine-tuned model: {model_id} (base: {base_model})")
    
    def _find_matching_fine_tuned_model(
        self,
        task_type: str,
        complexity: TaskComplexity
    ) -> Optional[str]:
        """Find a fine-tuned model matching the task"""
        if not self.fine_tuned_models:
            return None
        
        # Simple matching logic - can be enhanced
        for model_id, model_info in self.fine_tuned_models.items():
            metadata = model_info.get("metadata", {})
            best_for = metadata.get("best_for", [])
            
            # Check if model is suitable for this task
            if task_type.lower() in [tag.lower() for tag in best_for]:
                return model_id
        
        # Return first available fine-tuned model if any
        if self.fine_tuned_models:
            return list(self.fine_tuned_models.keys())[0]
        
        return None
    
    async def _fine_tuned_chat_completion(
        self,
        messages: List[Dict],
        model_id: str,
        max_tokens: int,
        ...
        """Handle fine-tuned model chat completion"""
        self.request_count["fine_tuned"] += 1
        
        if model_id not in self.fine_tuned_models:
            raise ValueError(f"Fine-tuned model not found: {model_id}")
        
        model_info = self.fine_tuned_models[model_id]
        base_model = model_info["base_model"]
        model_path = model_info.get("path")
        
        # Determine if it's a Claude or Ollama fine-tuned model
        if base_model.startswith("claude"):
            # Use Claude API with fine-tuned model ID
            if self.claude_client:
                claude_messages = []
                system_message = None
                ...
                # Use fine-tuned model ID (format: ft:model-id)
                fine_tuned_model_id = f"ft:{model_id}" if not model_id.startswith("ft:") else model_id
            ...
            if model_path and self.ollama_client:
                # Load fine-tuned model in Ollama if needed
                prompt = self._convert_messages_to_prompt(messages)
                
                response = await self.ollama_client.generate(
                    model=model_id,  # Use fine-tuned model name
                    prompt=prompt,
                    options={
                        "num_predict": max_tokens,
                        "temperature": temperature
        ...
        logger.warning(f"⚠️ Fine-tuned model {model_id} not available, falling back to base model {base_model}")
        if base_model.startswith("claude"):
            return await self._cloud_chat_completion(messages, "claude", max_tokens, temperature)
        else:
            return await self._local_chat_completion(messages, base_model, max_tokens, temperature)
    
    async def get_fine_tuned_models(self) -> Dict[str, Any]:
        """Get list of registered fine-tuned models"""
        return {
            "models": list(self.fine_tuned_models.keys()),
            "details": self.fine_tuned_models,
            "preference_enabled": self.fine_tuned_preference
        }
